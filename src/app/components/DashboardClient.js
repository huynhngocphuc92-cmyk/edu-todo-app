...
export default function DashboardClient({ userId }) {
  ...
  // Hàm tải danh sách tasks từ Supabase, chỉ lấy tasks của user hiện tại
  ...
  // Thêm task mới lên Supabase với mapping đúng tên cột và user_id
    ...
    const { error } = await supabase.from('tasks').insert([
      {
        ...
        deadline: deadline,
        completed: false,
        user_id: userId              // Gán user_id của user hiện tại
      }
    ]);
  ...
  const handleStatusToggle = async (id, currentStatus) => {
    ...
    await supabase
      ...
      .eq('id', id)
      .eq('user_id', userId); // Đảm bảo chỉ update task của user hiện tại
  ...
  const handleDeleteTask = async (id) => {
    ...
    await supabase
      ...
      .delete()
      .eq('id', id)
      .eq('user_id', userId); // Đảm bảo chỉ xóa task của user hiện tại
    setEditingIds(ids => ids.filter(x => x !== id));
    fetchTasks();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-10 relative">
      {/* Nút đăng xuất ở góc trên cùng bên phải */}
      <button
        onClick={handleLogout}
        className="absolute top-4 right-4 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-semibold shadow-md transition flex items-center gap-2"
      >
        <svg
          className="w-5 h-5"
          fill="none"
